...
def get_graphql_client(secret):
  global graphql_client
  if graphql_client is None:
    token_str = get_token(secret)
...
def create_secret(service_client, arn, token):
    ...
    try:
        ...
        generateNewToken = '''mutation createToken {
        ...
        # Put the secret
        service_client.put_secret_value(SecretId=arn, ClientRequestToken=token, SecretString=json.dumps(secret) , VersionStages=['AWSPENDING'])
...
def set_secret(service_client, arn, token):
    ...
    """
    logger.info("set_secret: secret was generated by sonrai so we don't have to push in")
...
def test_secret(service_client, arn, token):
    ...
    try:
        service_client.get_secret_value(SecretId=arn, VersionId=token, VersionStage="AWSPENDING")
        logger.info("test_secret: Successfully retrieved secret for %s." % arn)
    except service_client.exceptions.ResourceNotFoundException:
        logger.error("test_secret: Cannot find secret for ARN %s and version %s." % (arn, token))
